...
sub handler {
    ...
    my $file                   = "$docRoot/admin/$mappedFilename";
    ...
    if ($need_session) { 
        ...
        if (0
            || (!$need_login)
            || $session->{email} =~ /\S/
            || $mappedFilename eq "/admin/login.html"
            || $mappedFilename eq "/admin/processLogin.html"
            ) {
            ; # do nothing
        }
        else {
            ...
            $resp_headers->set("Location" => "/admin/login.html");
            $dbh->commit();
            $dbh->disconnect();
