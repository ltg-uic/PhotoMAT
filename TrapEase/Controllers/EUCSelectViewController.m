    ...
    [self.group enumerateAssetsUsingBlock:^(ALAsset *asset, NSUInteger index, BOOL *stop) {
        if (asset != nil) {
            if (burstIndex == -1) {
                ...
                EUCImage * image = [[EUCImage alloc] initWithIndex:index andFilename:asset.defaultRepresentation.filename];
                image.url = [asset valueForProperty:ALAssetPropertyAssetURL];
